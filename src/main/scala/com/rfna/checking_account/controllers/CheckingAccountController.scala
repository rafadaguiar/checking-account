...
trait CheckingAccountController extends CheckingAccountInternals with CheckingAccountMongoDAO with OperationsMongoDAO {
  ...
  def insertAccountOperations(accountId: String, request: Request) = {
    for {
      ...
      response <- doIfAccountIsDefined(accountId, account => Created(insertOperations(account.get, operations).asJson))
    } yield (response)
  }

  def getAccountBalance(accountId: String) = {
    doIfAccountIsDefined(accountId, account => Ok(getBalance(getOperations(account.get)).asJson))
  }

  def getAccountDebts(accountId: String) = {
    doIfAccountIsDefined(accountId, account => Ok(getPeriodsOfDebt(getOperations(account.get)).asJson))
  }
  ...
  private def doIfAccountIsDefined(accountId: String, f: Option[CheckingAccount] => Task[Response]): Task[Response] = {
